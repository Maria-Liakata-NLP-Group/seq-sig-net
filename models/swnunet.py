...
class SWNUNet(nn.Module):
    def __init__(
        ...
        hidden_dim_lstm :  list[int]
            A list that contains the dimensions of the hidden layer for the LSTM in the SWNU block.
        embedding_dim: int
        ...
        self.hidden_dim_lstm = hidden_dim_lstm if isinstance(hidden_dim_lstm, list) else [hidden_dim_lstm]
        ...
        self.tanh1 = nn.Tanh()
        #Signature with lift
        self.signature_first = signatory.LogSignature(depth=sig_d, stream=True)
        input_dim_lstm = signatory.logsignature_channels(output_channels, sig_d)
        
        #additional stacked units in the network
        if (len(self.hidden_dim_lstm) > 1):
            self.lstm_stack = []
            for stack in range(len(self.hidden_dim_lstm)-1):
                self.lstm_stack.append(nn.LSTM(input_size=input_dim_lstm, hidden_size=self.hidden_dim_lstm[stack], num_layers=1, batch_first=True, bidirectional=False).double())
                input_dim_lstm = signatory.logsignature_channels(self.hidden_dim_lstm[stack], sig_d)
            self.signature_stack = signatory.LogSignature(depth=sig_d, stream=True)     

        self.lstm = nn.LSTM(input_size=input_dim_lstm, hidden_size=self.hidden_dim_lstm[-1], num_layers=1, batch_first=True, bidirectional=False).double()
        input_dim_comb = signatory.logsignature_channels(self.hidden_dim_lstm[-1] ,sig_d)

        # combination method
        if comb_method=='concatenation':
            # input dimensions for FFN
            input_dim = input_dim_comb + self.embedding_dim + self.num_time_features
        elif comb_method=='gated_addition':
            input_gated_linear = input_dim_comb + self.num_time_features
        ...
        self.signature_last = signatory.LogSignature(depth=sig_d, stream=False)
    ...
    def forward(self, x):
        if ((len(self.hidden_dim_lstm)+1) >= x.shape[2]):
            raise ValueError("`hidden_dim_lstm` is too long for this history window. Either increase the history window or decrease the hidden_dim_lstm length")
        ...
        #Signature
        out = self.signature_first(out)

        #if more multiple stacked units
        if (len(self.hidden_dim_lstm) > 1):
            for stack in range(len(self.hidden_dim_lstm)-1):
                out, (_, _) = self.lstm_stack[stack](out)
                out = self.signature_stack(out)            
        #LSTM
        out, (_, _) = self.lstm(out)
        #Signature
        out = self.signature_last(out)

        #Combine Last Post Embedding
        if self.comb_method=='concatenation':
            if self.num_time_features > 0:
                out = torch.cat((out, x[:, self.input_channels:(self.input_channels+self.num_time_features),:].max(2)[0], x[:,(self.input_channels+self.num_time_features):, 0]), dim=1)
            else:
                out = torch.cat((out, x[:,(self.input_channels+self.num_time_features):, 0]), dim=1)
        elif self.comb_method=='gated_addition':
            if self.num_time_features > 0:
                out_gated = torch.cat((out, x[:, self.input_channels:(self.input_channels+self.num_time_features),:].max(2)[0]), dim=1)
            else:
                out_gated = out
            ...
            #concatenation with bert output
            out = torch.cat((out_gated, x[:,(self.input_channels+self.num_time_features):, 0]), dim=1 ) 
